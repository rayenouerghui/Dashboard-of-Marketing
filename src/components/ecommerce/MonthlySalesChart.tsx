...
export default function MonthlySalesChart({ stats }: { stats: AttractionLeadStats }) {
  ...
  const options: ApexOptions = {
    ...
    fill: { opacity: 1 },
    tooltip: { x: { show: false }, y: { formatter: (v: number) => `${v} leads` } },
  };
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white px-5 pt-5 dark:border-gray-800 dark:bg-white/[0.03] sm:px-6 sm:pt-6">
      <div className="mb-2 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-gray-800 dark:text-white/90">Monthly Leads</h3>
          <p className="mt-0.5 text-sm text-gray-500 dark:text-gray-400">Physical attraction leads by month</p>
        </div>
